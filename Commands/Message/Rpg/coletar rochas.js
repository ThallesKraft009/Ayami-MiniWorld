...
data.arvores.map(x => {
  mapa = mapa.replace(`${x.x}`, "⛰️")
      ...
      embed.setDescription(`Rochas Coletadas: ${data.madeiras}\n\n${mapa}`)

    }

  const salvar = async() => {
    userdb.rpg.blocos.pedra = userdb.rpg.blocos.pedra +1;
...
for (let i = 0; i < data.arvores.length; i++) {
  if (data.player.x === data.arvores[i].x) {
    ...
    .addComponents(
      new ButtonBuilder()
      .setEmoji("⬅️")
      .setCustomId("voltar-rocha")
      .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
      .setEmoji("➡️")
      .setCustomId("andar-rocha")
      .setStyle(ButtonStyle.Secondary)
    )
